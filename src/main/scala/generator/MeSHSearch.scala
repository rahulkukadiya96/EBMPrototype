...
object MeSHSearch {
  ...
  def searchSubjectHeading(text: String, queryBuilder: Seq[String] => Future[String]): Future[String] = {
    for {
      ids <- callApi(s"$BASE_URL/esearch.fcgi?db=$MESH_DB_NAME&term=cancer$API_KEY", extractIdFromXml)
